...
/**
 * Solid disc in `color` with the glyph knocked out of it, so whatever sits
 * behind the icon shows through the mark exactly. The glyph used to be painted
 * in a hardcoded #f66233, which only ever matched the tomato footer.
 *
 * The mask id is shared on purpose: every instance renders the same geometry,
 * so a duplicate id resolves to an identical mask and keeps these icons free of
 * hooks (and therefore usable from server components).
 */
const MASK_ID = "instagram-glyph-knockout";
    ...
    <div onClick={onClick} className="cursor-pointer">
      ...
      >
        <mask id={MASK_ID} maskUnits="userSpaceOnUse">
          ...
          />
          <circle fill="black" cx="682.2" cy="317.8" r="41" />
        </mask>
